...
discoveriesPerm <- sapply(c(1:length(rhos)), function(x) dI(ix = c(1:1000),cv = cv[,x],praw = Pvalues[,x]))

discoveriesHommel <- sapply(c(1:length(rhos)), function(x) discoveries(hommel(Pvalues[,x]),ix = c(1:1000)))

plot(rhos,discoveriesPerm,  type = "l", xlab = expression(rho), ylab = "Discoveries")
...
plot(R,(R-Bpar)/pmax(1,R), type="l", col='red', lwd=2, ylab = "upper bound TDP", xlab='number of rejection', ylim = c(0,1))
lines(R,(R-Bperm)/pmax(1,R), type="l", col=1, lwd=2)
lines(R,(R-V)/pmax(1,R), type="l", lwd=2, col = 'blue')
