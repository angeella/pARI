...
B <- 1000 #number of permutation
rho <- 0.2 #Level of equi correlation between pairs of variables
...
```{r}
pARI::plotNullDistribution(family = NULL,alpha = alpha, P = P,B = B)
```

We add the Simes critical values:

```{r}
pARI::plotNullDistribution(family = "simes", alpha = alpha, P = P,B = B, delta = delta)
```

```{r}
pARI::plotNullDistribution(family = "finner", alpha = alpha, P = P,B = B, delta = delta)
```
