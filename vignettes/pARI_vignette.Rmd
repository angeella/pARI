...
So, we represent the lower confidence bounds for the proportion of differentially expressed genes (TDP) inside each pathway in the following plot:

```{r, eval = FALSE}
db %>% dplyr::filter(!is.na(TDP) | TDP!=0) %>% dplyr::filter(size >10) %>% ggplot(aes(x = TDP, y = name, col = size, size = size)) + geom_point()
```
