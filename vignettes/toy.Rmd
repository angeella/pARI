...
We have $2910$ false null hypothesis. Using the hommel method by Goeman et al.:

```{r}
ix <- c(1:m)

res <- ARIpermutation::testByRandomization(X, B = 1000)

praw <- res$p
out1 <- hommel(praw)

hommel::discoveries(out1,ix,alpha = 0.05)
```
...
out3 <- sapply(R, function(x) ARIpermutation::discoveriesPerm(out,ix = c(1:x))[[1]])
