...
simEC <- function(m,rho,n,pi0,power,B, alpha){
  ...
  res <- signTest(X,B, rand = F, B = 1000,alternative = "two.sided")
  Test <- cbind(res$Test,res$Test_H0)
  P <- cbind(res$pv,res$pv_H0)
  lambdaSimes <- pARI::lambdaOpt(pvalues = P,family = family,alpha = alpha,delta = 0)
  cvSimes <- pARI::cv(pvalues = P,family = family,alpha = alpha,lambda = lambdaSimes,delta = 0)
...
discoveriesPerm <- sapply(c(1:length(rhos)), function(x) pARI::dI(ix = c(1:m),cv = cv[,x],praw = Pvalues[,x]))
